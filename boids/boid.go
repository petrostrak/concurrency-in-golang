...
func (b *Boid) borderBounce(pos, maxBoarderPos float64) float64 {
	if pos < viewRadius {
		return 1 / pos
	} else if pos > maxBoarderPos-viewRadius {
		return 1 / (pos - maxBoarderPos)
	}
	return 0
}
...
func (b *Boid) calcAcceleration() Vector2D {
	...
	if count > 0 {
		...
		accelAlignment := avgVelocity.Substract(b.velocity).MultiblyV(adjRate)
		accelCohesion := avgPosition.Substract(b.position).MultiblyV(adjRate)
		accelSeparation := separation.MultiblyV(adjRate)
